...
if (Test-Network) {
    ...
    if ($Script:Winget) {
        ...
        #Check if Winget Pin support is available
        ...
        #Load and apply pin configurations if pin support is available and pin management is enabled
        if ($Script:WingetPinSupported) {
            if ($WAUConfig.WAU_EnablePinManagement -eq 1) {
                Write-ToLog "WAU Pin Management is enabled" "Green"
                $Script:PinConfigs = Get-WAUPinConfig
                
                if ($Script:PinConfigs.Count -gt 0) {
                    Write-ToLog "Applying pin configurations before checking for updates..." "Yellow"
                    ...
                    if ($pinApplyResult) {
                        ...
                        Write-ToLog "Some pin configurations failed to apply" "Yellow"
                    }
                }
            }
            else {
